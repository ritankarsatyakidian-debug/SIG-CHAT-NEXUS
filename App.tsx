...
const LaptopScene = ({ onExit }: { onExit: () => void }) => {
  ...
  const checkCode = () => {
    // Check if the assembly matches correctOrder content (regardless of initial shuffle)
    ...
    <div className="absolute inset-0 bg-black flex flex-col items-center justify-center font-mono p-4">
       ...
       {!keyForged ? (
         ...
         <div className="flex flex-col items-center animate-fade-in relative z-10">
           <div 
             onClick={swipeCard}
             className="w-72 h-44 bg-gradient-to-br from-yellow-500 to-amber-700 rounded-xl shadow-[0_0_50px_rgba(234,179,8,0.4)] flex flex-col items-center justify-center mb-8 border-t border-yellow-300 relative overflow-hidden group cursor-pointer hover:scale-105 transition-transform"
           >
              {/* Card Chip */}
           ...
           <button 
            onClick={swipeCard} 
            className="px-8 py-3 bg-cyan-600 hover:bg-cyan-500 text-white rounded font-bold shadow-lg shadow-cyan-500/30 transition-all flex items-center"
           >
...
// Helper icon since CreditCardIcon isn't imported from solid
...
const MeetingRoom = ({ user, onClose }: { user: User, onClose: () => void }) => {
  ...
  // Participants including user and bots
  ...
  return (
    <div className="absolute inset-0 z-50 bg-[#202124] flex flex-col lg:flex-row text-white font-sans overflow-hidden">
      ...
      <div className="flex-1 flex flex-col p-2 md:p-4">
         ...
         <div className="h-16 md:h-20 flex items-center justify-center space-x-4 md:space-x-6 bg-[#202124] shrink-0">
            <button 
              ...
              className={`p-3 md:p-4 rounded-full transition-colors ${isListening ? 'bg-red-500 animate-pulse' : 'bg-[#3c4043] hover:bg-[#474a4d]'}`}
            >
      ...
      {/* Side Chat - Collapsible or stacked on mobile */}
...
const TerminalWindow = ({ user, onClose, onUpdateUser }: { user: User, onClose: () => void, onUpdateUser: (u: User) => void }) => {
  ...
  const handleCommand = (e: React.KeyboardEvent) => {
    if (e.key === 'Enter') {
      ...
      if (cmd === 'EXIT') {
        ...
        setHistory([]);
      } else if (cmd === 'POWERLINGX') {
        setScene('BROWSER');
        setHistory([]);
      } else if (cmd === 'TAIQ') {
        setScene('STUDY');
        setHistory([]);
      } else if (cmd === 'BEL-IQ-Z') {
        setScene('LAB');
        setHistory([]);
      } else if (cmd === 'CAPTAIN-COSMIC') {
        setScene('LAPTOP');
        setHistory([]);
      } else {
        newHistory.push(`Access Denied: Unknown Command "${cmd}"`);
  ...
  const handleBrowserGo = () => {
    if (url.trim().toUpperCase() === 'POWERLIEX') {
      setShowConstitution(true);
    } else {
      alert("404: Node Not Found on Sigmax Grid");
    }
  };
  ...
  if (scene === 'LAPTOP') {
    return <LaptopScene onExit={() => setScene('TERMINAL')} />;
  }
        ...
        <div className="h-12 bg-slate-800 flex items-center px-4 space-x-2 border-b border-slate-700">
           ...
           <div className="flex-1 bg-slate-900 rounded-full h-8 px-4 flex items-center">
             ...
             <input 
              ...
              placeholder="Enter Grid Address..."
             />
           </div>
                ...
                <div className="max-w-2xl mx-auto border-4 border-double border-slate-900 p-4 md:p-8 shadow-2xl bg-[#fffdf0]">
                  ...
                  <div className="border-t border-slate-900 pt-8 mt-8 text-center">
                    ...
                    <button 
                      ...
                      className="px-6 py-3 md:px-8 md:py-4 bg-slate-900 text-white font-bold text-lg md:text-xl hover:bg-slate-700 transition-colors font-sans"
                    >
                      SIGN PLEDGE
                    </button>
                  </div>
    ...
    return (
      <div className="absolute inset-0 z-50 bg-[#1a1a1a] flex items-center justify-center relative overflow-hidden">
        ...
        <div className="relative z-10 bg-[#3d2b1f] p-4 md:p-8 rounded-lg shadow-2xl border-t-4 border-[#5c4033] flex flex-col items-center">
           ...
           <div 
            onClick={() => alert("The TAIQ Constitution emphasizes Logic, Code, and Order above all emotion.")}
            className="cursor-pointer bg-[#f3e5ab] w-48 h-64 md:w-64 md:h-80 p-6 shadow-lg transform hover:scale-105 transition-transform border border-amber-900/20"
           >
    ...
    return (
      <div className="absolute inset-0 z-50 bg-slate-900 text-cyan-500 font-mono p-4 md:p-8 overflow-y-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-8 h-full">
          ...
          {/* Section 2: Interactive Software */}
          <div className="border border-cyan-500/30 p-6 rounded-lg bg-slate-800/50">
             ...
             <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <button onClick={() => alert("Software by Ibhan: Neural Net Optimizer v9.2 - OPERATIONAL")} className="p-3 bg-slate-700 hover:bg-cyan-900 rounded text-xs text-left">
                   <ComputerDesktopIcon className="w-4 h-4 inline mr-2"/> Ibhan's OptiNet
                </button>
                <button onClick={() => alert("Software by Ritankar: Quantum Encryption Keygen - SECURE")} className="p-3 bg-slate-700 hover:bg-cyan-900 rounded text-xs text-left">
                   <ComputerDesktopIcon className="w-4 h-4 inline mr-2"/> Ritankar's Keygen
                </button>
                <button onClick={() => alert("Software by Satyaki: Defense Grid Monitor - ACTIVE")} className="p-3 bg-slate-700 hover:bg-cyan-900 rounded text-xs text-left">
                   <ComputerDesktopIcon className="w-4 h-4 inline mr-2"/> Satyaki's Shield
                </button>
                <button onClick={() => alert("Software by Dian: Holographic UI Render Engine - RENDERING")} className="p-3 bg-slate-700 hover:bg-cyan-900 rounded text-xs text-left">
                   <ComputerDesktopIcon className="w-4 h-4 inline mr-2"/> Dian's HoloCore
                </button>
                <button onClick={() => alert("Software by Soumyadeepta: Global Comms Uplink - CONNECTED")} className="p-3 bg-slate-700 hover:bg-cyan-900 rounded text-xs text-left">
                   <ComputerDesktopIcon className="w-4 h-4 inline mr-2"/> Soumya's Uplink
                </button>
             </div>
          </div>
        </div>
    ...
    <div className="absolute inset-0 z-50 bg-black text-green-500 font-mono p-4 md:p-8 overflow-hidden flex flex-col">
       ...
       <div className="flex items-center bg-green-900/20 p-2 rounded">
         ...
         <input 
           ...
           className="bg-transparent border-none outline-none text-green-500 w-full text-sm md:text-base"
         />
       </div>
    </div>
  );
...
const VerificationFlow = ({ user, onComplete }: { user: User, onComplete: (u: User) => void }) => {
  ...
  const captureAndAnalyze = async () => {
    ...
    setTimeout(() => {
      ...
      setMatchResult(result.name); // Null if not matched
      
      // Save verification
      const verifiedUser = Persistence.verifyUser(
        user.id, 
        `REP-${Date.now()}`, 
        result.name
      );
       ...
       <div className="relative z-10 w-full max-w-lg p-6 md:p-8 glass-panel rounded-2xl border border-cyan-500/30">
           ...
           <div className="space-y-4 animate-fade-in">
             <div className="bg-slate-800/50 p-4 rounded-lg border border-slate-700">
               ...
               <input 
                 ...
                 disabled 
                 className="w-full bg-slate-900/50 border border-slate-700 rounded p-2 text-slate-400 mb-2" 
               />
               <input 
                 type="number" 
                 placeholder="Biological Age"
                 className="w-full bg-slate-900/50 border border-slate-700 rounded p-2 text-white mb-2" 
                 onChange={e => setReportData({...reportData, age: e.target.value})}
               />
               <select 
                 className="w-full bg-slate-900/50 border border-slate-700 rounded p-2 text-white"
             ...
             <button 
               onClick={() => setStep(2)}
               disabled={!reportData.age}
               className="w-full py-3 bg-cyan-700 hover:bg-cyan-600 text-white font-bold rounded shadow-lg shadow-cyan-500/30 transition-all uppercase disabled:opacity-50"
             >
         ...
         {step === 2 && (
           <div className="flex flex-col items-center">
             ...
             {scanStatus === 'idle' && (
               <button onClick={captureAndAnalyze} className="flex items-center space-x-2 px-6 py-3 bg-cyan-600 rounded-full text-white font-bold hover:bg-cyan-500 transition-all">
                 <CameraIcon className="w-5 h-5" /> <span>Initiate Face Scan</span>
               </button>
             )}

             {scanStatus === 'analyzing' && (
               <p className="text-cyan-400 animate-pulse text-sm uppercase tracking-widest">Processing Neural Vector...</p>
             )}
    ...
    try {
      if (isLogin) {
        ...
        const newUser = Persistence.signup({
          ...
          bio: 'New recruit to the Sigmax Alliance.',
    ...
    <div className="flex h-screen w-screen items-center justify-center bg-black relative overflow-hidden">
      <div className="absolute inset-0 z-0">
        ...
        <div className="grid grid-cols-12 h-full w-full opacity-10 pointer-events-none">
           {Array.from({length: 12}).map((_,i) => <div key={i} className="border-r border-cyan-500/20 h-full"></div>)}
        </div>
      </div>
      ...
      <div className="z-10 w-full max-w-md p-8 glass-panel rounded-2xl border border-cyan-500/30 shadow-2xl shadow-cyan-900/50 mx-4">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          {!isLogin && (
            <>
              <div>
                <label className="block text-xs text-slate-400 uppercase mb-1">Identity Name</label>
                <input 
                  type="text" 
                  required 
                  className="w-full bg-slate-900/50 border border-slate-700 rounded p-2 text-white focus:border-cyan-500 outline-none"
                  value={formData.name}
              ...
              <div>
                ...
                <select 
                  ...
                  onChange={(e) => setFormData({...formData, country: e.target.value as SigmaxCountry})}
                >
          ...
          <div>
            <label className="block text-xs text-slate-400 uppercase mb-1">Comm Link (Phone No.)</label>
            <input 
              type="text" 
              required 
          ...
          <div>
            <label className="block text-xs text-slate-400 uppercase mb-1">Access Key (Password)</label>
            <input 
              type="password" 
              required 
              className="w-full bg-slate-900/50 border border-slate-700 rounded p-2 text-white focus:border-cyan-500 outline-none"
          ...
          <button 
            type="submit"
            className="w-full py-3 mt-6 bg-cyan-700 hover:bg-cyan-600 text-white font-bold rounded-lg shadow-lg shadow-cyan-500/30 transition-all uppercase tracking-wider text-sm flex justify-center items-center"
          >
        ...
        <div className="mt-6 text-center">
          ...
          >
            {isLogin ? "Request New Clearance Level (Sign Up)" : "Return to Login Node"}
          </button>
        </div>
      </div>
    </div>
  ...
  return (
    <div className={`flex ${isOwn ? 'justify-end' : 'justify-start'} mb-6 group relative`}>
      <div className={`max-w-[85%] md:max-w-[75%] rounded-2xl p-3 md:p-4 relative overflow-visible backdrop-blur-sm
        ${isOwn 
          ? 'bg-gradient-to-br from-indigo-600/80 to-purple-700/80 text-white rounded-tr-none border border-indigo-400/30' 
          : 'bg-slate-800/80 text-slate-200 rounded-tl-none border border-slate-600/30'
        } ${message.priority === 'CRITICAL' ? 'border-red-500 animate-pulse bg-red-900/40' : ''}`}
        onMouseEnter={() => setShowEmoji(true)}
        onMouseLeave={() => setShowEmoji(false)}
        onClick={() => setShowEmoji(!showEmoji)} // Allow tap on mobile to show emoji
      >
        ...
        <div className="relative z-10 text-sm md:text-base whitespace-pre-wrap break-words">
          {message.text}
          {translated && (
            <div className="mt-2 pt-2 border-t border-white/10 text-xs text-emerald-300 italic">
...
export default function App() {
  ...
  const [showManageChat, setShowManageChat] = useState(false); // New state for admin modal
  ...
  useEffect(() => {
    ...
    Persistence.subscribeToNetwork(
      (newMessage) => {
        // Refresh messages if this is the active chat
        if (activeChatId === newMessage.chatId) {
          // Check if it's an update (reaction) or new
          setActiveMessages(prev => {
             const exists = prev.findIndex(m => m.id === newMessage.id);
             if (exists !== -1) {
               const updated = [...prev];
               updated[exists] = newMessage;
               return updated;
             }
  ...
  useEffect(() => {
    ...
    if (lastMsg.senderId !== user?.id) {
       GeminiService.generateSmartReplies(
         lastMsg.text, 
         `In chat: ${activeChat?.name || 'Private Chat'}. My role: ${user?.role}`
  ...
  const handleSendMessage = async (text: string, priority: 'NORMAL' | 'HIGH' | 'CRITICAL' = 'NORMAL') => {
    ...
    if (activeChatId === 'admin_sigmax' && text === '/TERMINAL') {
      setShowTerminal(true);
      setInputText('');
      return;
    }

    if (text === 'LING-DYNOMAX') {
    ...
    try {
      const newMessage: Message = {
        ...
        status: 'sent',
        priority: priority,
        metadata: { networkType: 'SECURE_MESH' }
      };
      ...
      // Persona Response logic for bots
      const activeChat = chats.find(c => c.id === activeChatId);
      if (activeChat?.type === 'private') {
        ...
        if (otherUser && otherUser.systemPrompt) {
          setTimeout(async () => {
            const responseText = await GeminiService.generatePersonaResponse(
              activeMessages.concat(newMessage),
              otherUser.name,
              otherUser.systemPrompt,
              text
            );
      ...
      alert(e.message); // Handle blocked errors
      setActiveMessages(prev => prev.slice(0, -1)); // Revert optimistic update
    }
  };
  ...
  const handleBlockUser = (targetId: string) => {
    ...
    if (isBlocked) {
      Persistence.unblockUser(user.id, targetId);
    } else {
      Persistence.blockUser(user.id, targetId);
    }
  ...
  const handleAddMember = () => {
    ...
    try {
      Persistence.addMemberToChat(activeChatId, user.id, manageAddPhone);
      setManageAddPhone('');
      loadData(user.id); // Refresh user map/chats
    } catch (e: any) {
      alert(e.message);
    }
  };
  ...
  const handleRemoveMember = (targetId: string) => {
    ...
    if (window.confirm("Are you sure you want to remove this entity from the secure channel?")) {
      try {
        Persistence.removeMemberFromChat(activeChatId, user.id, targetId);
        loadData(user.id);
      } catch (e: any) {
        alert(e.message);
      }
  ...
  const runAnalysis = async () => {
    ...
    setIsAnalyzing(true);
    const result = await GeminiService.analyzeConversation(activeMessages);
    setAnalysis(result);
  ...
  const ChatItem: React.FC<ChatItemProps> = ({ chat, isActive, onClick, otherUser }) => {
    ...
    if (chat.type === 'private' && otherUser) {
      ...
      theme = COUNTRY_THEMES[otherUser.country];
    } else {
      theme = { color: 'text-slate-300', accent: 'border-slate-500', icon: chat.type === 'channel' ? <HashtagIcon className="w-4 h-4"/> : <UserGroupIcon className="w-4 h-4"/> };
    }
  ...
  if (!user) {
    return <AuthScreen onLogin={(u) => { setUser(u); loadData(u.id); }} />;
  }

  // Verification Check
  if (!user.isVerified) {
    return <VerificationFlow user={user} onComplete={(u) => { setUser(u); loadData(u.id); }} />;
  }
  ...
  // Meeting Room
  if (showMeetingRoom) {
    return <MeetingRoom user={user} onClose={() => setShowMeetingRoom(false)} />;
  }
  ...
  // Check if user is admin of active chat
  const isAdmin = activeChat?.admins?.includes(user.id);

  // Filter messages based on blocks
  ...
  return (
    <div className="flex h-screen w-screen overflow-hidden bg-[radial-gradient(ellipse_at_top_right,_var(--tw-gradient-stops))] from-slate-900 via-[#0f172a] to-black">
      ...
      <div className={`fixed inset-y-0 left-0 z-40 w-80 border-r border-slate-700/50 flex flex-col glass-panel backdrop-blur-xl transition-transform duration-300 ease-in-out lg:translate-x-0 lg:static lg:inset-auto ${mobileMenuOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="p-4 border-b border-slate-700/50">
          <div className="flex justify-between items-start mb-4">
            <div className="flex items-center space-x-3">
              ...
              <div>
                <h1 className="font-bold text-lg tracking-wider text-white">SIGMAX</h1>
                <p className="text-[10px] text-cyan-400 uppercase tracking-widest">{user.country} NODE</p>
              </div>
            </div>
          ...
          <button 
            onClick={() => { setShowAddChat(true); setMobileMenuOpen(false); }}
            className="w-full flex items-center justify-center space-x-2 py-2 bg-slate-800/80 hover:bg-cyan-700/50 border border-slate-600 hover:border-cyan-500 rounded-lg text-xs text-slate-300 transition-all uppercase tracking-wider"
          >
            <PlusIcon className="w-4 h-4" /> <span>Manage Connections</span>
          </button>
        </div>

        <div className="flex-1 overflow-y-auto">
          {/* Channels Section */}
          <div className="px-4 py-2 mt-2 text-[10px] font-bold text-slate-500 uppercase tracking-wider flex items-center">
          ...
          {/* Messages Section */}
          <div className="px-4 py-2 mt-4 text-[10px] font-bold text-slate-500 uppercase tracking-wider flex items-center">
             <SignalIcon className="w-3 h-3 mr-1" /> Direct Links
          </div>
          {chats.filter(c => c.type !== 'channel').map(chat => {
        ...
        <div className="p-4 border-t border-slate-700/50 bg-slate-900/50 text-xs text-slate-500 flex justify-between items-center">
           <div className="flex flex-col">
              <span>{user.name}</span>
              <span className="text-cyan-500">{user.verificationData?.identityMatch ? 'ADMIN' : 'USER'}</span>
           </div>
           <button onClick={handleLogout} className="lg:hidden text-slate-500 hover:text-red-400"><LockClosedIcon className="w-5 h-5"/></button>
        </div>
      </div>
      ...
      {activeChatId && activeChat ? (
        <div className="flex-1 flex flex-col relative w-full">
          ...
          <div className="h-16 border-b border-slate-700/50 glass-panel flex justify-between items-center px-4 md:px-6 z-10">
            <div className="flex items-center space-x-3 md:space-x-4 overflow-hidden">
               {/* Mobile Menu Button */}
              <button onClick={() => setMobileMenuOpen(true)} className="lg:hidden text-slate-400 hover:text-white shrink-0">
                <Bars3Icon className="w-6 h-6" />
              </button>

              <div className="flex flex-col md:flex-row md:items-center">
                 <h2 className={`text-lg md:text-xl font-semibold tracking-wide text-white truncate max-w-[150px] md:max-w-xs`}>{activeChatName}</h2>
                 <div className="flex items-center space-x-2 mt-1 md:mt-0 md:ml-4">
                    {activeChat.type === 'private' && otherChatUser && (
                      <span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider border ${activeChatTheme?.accent} ${activeChatTheme?.color} bg-slate-900/50`}>
                        {otherChatUser.country}
                      </span>
                    )}
                    {activeChat.type === 'channel' && (
                       <span className="px-2 py-0.5 rounded text-[10px] font-bold bg-cyan-900/50 text-cyan-400 border border-cyan-700 shrink-0">SECURE CHANNEL</span>
                    )}
                 </div>
              </div>
            </div>
            <div className="flex items-center space-x-2 md:space-x-4 shrink-0">
              {isAdmin && activeChat.type !== 'private' && (
                 <button 
                   onClick={() => setShowManageChat(true)}
                   className="p-2 hover:bg-slate-700/50 rounded-full transition-colors text-slate-400 hover:text-cyan-400" 
                   title="Manage Channel"
                 >
                   <Cog6ToothIcon className="w-5 h-5 md:w-6 md:h-6" />
                 </button>
              )}
              <button onClick={runAnalysis} className="p-2 hover:bg-slate-700/50 rounded-full transition-colors text-cyan-400" title="Analyze">
          ...
          <div 
            ref={scrollRef}
            className="flex-1 overflow-y-auto p-4 md:p-6 space-y-2 bg-[url('https://grainy-gradients.vercel.app/noise.svg')] bg-opacity-5"
          >
          ...
          {smartSuggestions.length > 0 && (
            <div className="px-4 md:px-6 py-2 flex space-x-2 overflow-x-auto bg-slate-900/40 border-t border-slate-800/50">
              {smartSuggestions.map((suggestion, idx) => (
                <button
                  key={idx}
                  onClick={() => handleSendMessage(suggestion.text)}
                  className="whitespace-nowrap px-4 py-2 rounded-xl bg-slate-800/80 border border-slate-700 hover:border-cyan-500/50 hover:bg-slate-700 transition-all text-xs text-slate-300 flex flex-col items-start"
                >
                  <span className="font-semibold text-cyan-400 mb-0.5 uppercase text-[9px]">{suggestion.tone}</span>
                  {suggestion.text}
                </button>
              ))}
            </div>
          ...
          <div className="p-2 md:p-4 glass-panel border-t border-slate-700/50">
            <div className="flex items-center space-x-2 md:space-x-4 bg-slate-800/50 rounded-2xl p-2 border border-slate-600/50 focus-within:border-cyan-500/50 transition-colors">
              <button 
                onClick={() => handleSendMessage(inputText, 'CRITICAL')}
                className="p-2 rounded-full hover:bg-red-500/20 text-slate-400 hover:text-red-500 transition-colors shrink-0"
                title="Send Critical Priority"
              >
              ...
              <input 
                ...
                className="flex-1 bg-transparent border-none focus:ring-0 text-slate-200 placeholder-slate-500 min-w-0"
              />
              <button 
                onClick={() => handleSendMessage(inputText)}
                disabled={!inputText.trim()}
                className="p-2 bg-cyan-600 hover:bg-cyan-500 rounded-xl text-white disabled:opacity-50 disabled:cursor-not-allowed transition-all shadow-lg shadow-cyan-500/20 shrink-0"
              >
        ...
        <div className="flex-1 flex flex-col items-center justify-center text-slate-600 bg-slate-900/50 relative w-full">
           {/* Mobile Menu Button for empty state */}
           <button onClick={() => setMobileMenuOpen(true)} className="lg:hidden absolute top-4 left-4 text-slate-400 hover:text-white p-2">
      ...
      <div className={`fixed inset-y-0 right-0 z-40 w-72 glass-panel border-l border-slate-700/50 flex flex-col p-6 overflow-y-auto transition-transform duration-300 lg:translate-x-0 lg:static lg:inset-auto ${showInfoPanel ? 'translate-x-0' : 'translate-x-full'} ${(!activeChat || activeChat.type !== 'private' || !otherChatUser) ? 'hidden' : 'lg:flex'}`}>
        {/* Mobile Close Button */}
        <button onClick={() => setShowInfoPanel(false)} className="lg:hidden absolute top-4 right-4 text-slate-400 hover:text-white">
        ...
        {activeChat && activeChat.type === 'private' && otherChatUser && (
          <>
            <div className="flex flex-col items-center mb-8 mt-6 lg:mt-0">
               <div className="w-24 h-24 rounded-full p-1 bg-gradient-to-br from-slate-700 to-slate-800 border border-slate-600 mb-4">
                  <img src={otherChatUser.avatar} alt="" className="w-full h-full rounded-full" />
               </div>
               ...
               {/* Block Button */}
               <button 
                 onClick={() => handleBlockUser(otherChatUser.id)}
                 className={`mt-4 flex items-center px-4 py-2 rounded text-xs font-bold uppercase transition-colors ${
                   user.blockedUserIds?.includes(otherChatUser.id) 
               ...
               >
                 ...
                 {user.blockedUserIds?.includes(otherChatUser.id) ? 'Unblock User' : 'Block User'}
               </button>
            </div>
            
            {analysis && (
               <div className="p-3 rounded-lg bg-slate-800/40 border border-slate-700">
                  <div className="text-[10px] uppercase text-slate-500 mb-1">Live Analysis</div>
                  <p className="text-xs text-slate-300">{analysis.summary}</p>
                  <div className={`mt-2 text-xs font-bold ${analysis.threatLevel === 'HIGH' ? 'text-red-400' : 'text-emerald-400'}`}>Threat: {analysis.threatLevel}</div>
               </div>
            )}
          </>
        )}
          ...
          <div className="w-full max-w-sm glass-panel p-6 rounded-2xl border border-slate-600">
            ...
            <div className="flex space-x-1 mb-4 bg-slate-800 rounded p-1">
              <button 
                ...
                className={`flex-1 py-2 md:py-1 text-[10px] rounded uppercase transition-colors ${modalMode === 'private' ? 'bg-cyan-700 text-white' : 'text-slate-400 hover:text-white'}`}
              >
                Private
              </button>
              <button 
                onClick={() => setModalMode('group')}
                className={`flex-1 py-2 md:py-1 text-[10px] rounded uppercase transition-colors ${modalMode === 'group' ? 'bg-cyan-700 text-white' : 'text-slate-400 hover:text-white'}`}
              >
                Group
              </button>
              <button 
                onClick={() => setModalMode('block')}
                className={`flex-1 py-2 md:py-1 text-[10px] rounded uppercase transition-colors ${modalMode === 'block' ? 'bg-red-900/80 text-white' : 'text-slate-400 hover:text-red-400'}`}
              >
                Block ID
              </button>
            </div>
            ...
            {modalMode === 'block' ? (
              <div className="mb-4">
                <p className="text-xs text-red-400 mb-2">Warning: Blocking a neural link will prevent all incoming data transmission from this source.</p>
                 <input 
                  type="text" 
                  placeholder="Target Comm Link (+SIG-...)"
                  className="w-full bg-slate-900 border border-red-900/50 focus:border-red-500 rounded p-2 text-white text-sm"
                  value={newChatPhone}
              ...
              <input 
                type="text" 
                placeholder="Target Comm Link (Phone No)"
                className="w-full bg-slate-900 border border-slate-700 rounded p-2 text-white text-sm mb-4"
                value={newChatPhone}
                onChange={e => setNewChatPhone(e.target.value)}
              />
            ) : (
              <>
                ...
                <input 
                  type="text" 
                  placeholder="Comm Links (comma separated)"
                  className="w-full bg-slate-900 border border-slate-700 rounded p-2 text-white text-sm mb-4"
                  value={newGroupData.phones}
                  onChange={e => setNewGroupData({...newGroupData, phones: e.target.value})}
                />
              </>
            )}
            ...
            <div className="flex justify-end space-x-2">
              ...
              <button 
                ...
                className={`px-4 py-2 rounded text-xs text-white font-bold transition-colors ${modalMode === 'block' ? 'bg-red-700 hover:bg-red-600' : 'bg-cyan-600 hover:bg-cyan-500'}`}
              >
                {modalMode === 'block' ? 'TERMINATE LINK' : 'CONNECT'}
              </button>
            </div>
          </div>
        </div>
        ...
        <div className="absolute inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
           <div className="w-full max-w-sm glass-panel p-6 rounded-2xl border border-cyan-500/50">
             <div className="flex justify-between items-center mb-4">
                ...
                <button onClick={() => setShowManageChat(false)} className="text-slate-500 hover:text-white">X</button>
             </div>
                 ...
                 return (
                   <div key={partId} className="flex justify-between items-center bg-slate-800/50 p-2 rounded">
                     <div className="flex items-center space-x-2 overflow-hidden">
                       <img src={partUser.avatar} className="w-6 h-6 rounded-full" />
                       <span className="text-xs text-slate-300 truncate">{partUser.name} {isSelf && '(You)'}</span>
                     </div>
                     {!isSelf && (
                       <button 
                        onClick={() => handleRemoveMember(partId)}
             ...
             <div className="border-t border-slate-700 pt-4">
               ...
               <div className="flex space-x-2">
                  <input 
                    type="text" 
                    placeholder="Comm Link (+SIG-...)"
                    value={manageAddPhone}
                    onChange={e => setManageAddPhone(e.target.value)}
